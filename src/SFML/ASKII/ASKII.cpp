...
{
    //if(cur_image.loadFromFile("../input/"+filename)){
      //  std::cout<<"Yay, image loaded\n";
    //}
...
void ASKII_ALGORITHM::convert_to_aski(std::string path) {
    askii_algorithm(path);
}
...
void ASKII_ALGORITHM::askii_algorithm(std::string path){
    std::vector<std::vector<std::string>> file;
    cur_image.loadFromFile(path);
    //std::ostream file(res);
